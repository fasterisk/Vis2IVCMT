function ParseString(sSubString)
{
  ...
  if (bMaybeNode == true)
  {
    ...
    // process string. for each [, increment iBrackets, for each ], decrement - if iBrackets==0,
	...
	// extract the substring for the child node, without the edge length information
	var sChildSubString = sSubString.substr(iPosOfSemikolon+1, i - iPosOfSemikolon - 1);
	
	// extract the length information as substring
	var sLengthOfEdgeString = sSubString.substr(iStartPos+1, iPosOfSemikolon - iStartPos-1);
  ...
  else if (bMaybeLeaf == true)
  {
    ...
    // node has no method for this at the moment, maybe:
    
    // rNewNode.SetValue(sSubString);
  }
...
{
  // TODO: read from file (I think the user must explicit upload the files, to process it in
